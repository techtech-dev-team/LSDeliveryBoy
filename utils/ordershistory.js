...
export const fetchAllOrders = async ({ page = 1, limit = 10, startDate, endDate, token }) => {
	try {
		...
		const assignedRes = await fetch(
			`${API_CONFIG.getBaseURL()}/delivery/deliveries${params}`,
			{
				method: 'GET',
				headers: {
		...
		const historyRes = await fetch(
			`${API_CONFIG.getBaseURL()}/delivery/history${params}`,
			{
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
		...
		const pagination = {
			...
			total: (assignedData.data?.pagination?.total || 0) + (historyData.data?.pagination?.total || 0)
		};
